...
import { DeployTutorial } from './deploy-tutorial';
...
export function DeploySection({ appInfo, deployInfo, domain, sshPort }: DeploySectionProps) {
  ...
  return (
    <>
      <DeployTutorial targetRef={sectionRef} />
      <div ref={sectionRef} className={styles.deploySection}>
        ...
        <Box className={styles.codeBlock}>
          <div className={styles.codeBlockContent}>
            ...
            {sshPort != null && sshPort !== 22 && (
              <div>
                <span className={styles.command}>$ git config --local</span> core.sshCommand
                &quot;ssh -p {sshPort}&quot;
              </div>
            )}
          </div>
